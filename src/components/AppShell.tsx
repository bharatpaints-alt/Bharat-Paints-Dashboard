...
export function AppShell({ page, setPage, children, online }: { page: PageId; setPage: (page: PageId) => void; children: React.ReactNode; online: boolean }) {
  return <div className="app-shell">
    ...
    <aside className="desktop-nav"><div className="desktop-title"><Boxes size={20} /> Employee Tools</div>{nav.map(({ id, label, icon: Icon }) => <button key={id} className={page === id ? 'active' : ''} onClick={() => setPage(id)}><Icon size={20} />{label}</button>)}</aside>
    <main>{children}</main>

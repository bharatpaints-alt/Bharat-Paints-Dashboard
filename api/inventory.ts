...
type ApiResponse<T> = { ok: true; data: T; error: null } | { ok: false; data: null; error: ApiError }
...
function success<T>(data: T): ApiResponse<T> { return { ok: true, data, error: null } }
function failure(code: string, message: string): ApiResponse<never> { return { ok: false, data: null, error: { code, message } } }
...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    if (action === 'getStock') return response.status(200).json(success(await getStock()))
    const allowed = new Set(['getProductImages', 'getProductImageData', 'uploadProductImage', 'deleteProductImage', 'getAppSettings'])
    if (!allowed.has(action)) return response.status(400).json(failure('UNKNOWN_ACTION', 'Unknown inventory action.'))
    return response.status(200).json(success(await callGas(action, payload)))
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unexpected server error.'
    return response.status(500).json(failure('REQUEST_FAILED', message))
  }
}
